...
class EigenSolverApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Eigen Solver")
    ...
    def create_ui(self):
        tk.Label(self.root, text="🔷 Eigenvalue & Eigenvector Solver",
    ...
    def solve(self):
        try:
            ...
            self.result.insert(tk.END, "Eigenvalues:\n")
            for v in values:
                self.result.insert(tk.END, f"{v}\n")

            self.result.insert(tk.END, "\nEigenvectors:\n")
            for vec in vectors.T:
                self.result.insert(tk.END, f"{vec}\n")
