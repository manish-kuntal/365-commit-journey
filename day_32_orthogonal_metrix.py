...
class OrthogonalCheckerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Orthogonal Matrix Checker")
    ...
    def create_ui(self):
        tk.Label(self.root, text="🔷 Orthogonal Matrix Checker",
        ...
        tk.Button(self.root, text="Check Orthogonal",
                  bg="#22c55e", fg="white",
    ...
    def check(self):
        try:
            ...
            A = np.array(matrix)
            I = np.eye(A.shape[0])
            check = np.allclose(A.T @ A, I)

            if check:
                self.result.config(text="✅ This matrix is ORTHOGONAL")
            else:
                self.result.config(text="❌ This matrix is NOT orthogonal")
